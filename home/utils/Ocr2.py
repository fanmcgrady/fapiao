...
def surface(filename, type='blue'):
    filepath = os.path.join('/Users/fangzhiyang/pic', filename)
    ...
    if type == None:
        ...
        if type == 'blue' or type == 'excess':
            ...
            out_filename = os.path.join('/Users/fangzhiyang/pic', out_filename)

            midProcessResult[0] = out_filename
            midProcessResult[1] = 2  # 专票
            # 暂时用原图，不用校正后的图
            midProcessResult[2] = textline(filepath)
    # 行提取
    blueTemplet = {
    ...
    if midProcessResult[1] == 2:
        rate = 2.0
        print("rate : 2.0")
        box = Detect.detect(cv2.imread(filepath), rate)
        # print( box.tolist())
